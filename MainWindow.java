...
public class MainWindow extends JFrame {
    ...
    // Variables are now declared locally inside their respective ActionListeners.
    
    private DefaultTableModel model; // Instance variable, not static

    private static double fixedInvestor(double principal, double rate, int years) {
        /*
         * Simulates compound interest with a fixed rate. Complexity: O(T) - Linear Time
         * Returns: (balance)
         */
    ...
    private double variableInvestor(double principal, double[] rate_list) {
        /*
         * Simulates growth based on a list of changing rates. Complexity: O(N) - Linear
         * Time based on list size Returns: (balance) """
         */
        double balance = 0;
        balance = principal;
    ...
    public static double maximumExpensed(double balance, double rate, int targetYears) {
        ...
        /*
         * OPTIMIZATION ALGORITHM: Binary Search (Divide & Conquer). Finds the optimal
         * withdrawal amount to last exactly 'target_years'.
    ...
    public MainWindow() {
        ...
        // Creates a main JPanel object with a primary constructor that accepts a
        // CardLayout
        JPanel mainPanel = new JPanel(cardLayout);

        // Creates JPanel objects with a primary constructor that accepts a new
        // GridBagLayout
        ...
        // These buttons when clicked allows the mainPanel to use the CardLayout to switch
        // between pages
        ...
        calculateFixedGrowth.addActionListener(e -> {
            ...
            // FIX #1: Local variables for safety
            double localPrincipal = 0;
            double localRate = 0;
            int localYears = 0;
            double localEndBalance = 0;

            // Validation Check to see if one or more fields are empty.
            if (initialValueField.getText().compareTo("") == 0 || interestField.getText().compareTo("") == 0
                    || yearField.getText().compareTo("") == 0) {
                JOptionPane.showMessageDialog(fixedGrowthPage,
                        "One or more fields were not filled\nPlease check to see you entered all values.", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }

            try {
                localPrincipal = Double.parseDouble(initialValueField.getText());
                localRate = Double.parseDouble(interestField.getText());
                localYears = Integer.parseInt(yearField.getText());
            } catch (NumberFormatException nf) {
                JOptionPane.showMessageDialog(fixedGrowthPage, "Invalid datatype. All fields must be in number format.",
            ...
            if (localPrincipal <= 0) {
                JOptionPane.showMessageDialog(fixedGrowthPage,
                        "Initial Investment must be a postive number (numbers greater than 0)", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (localRate <= 0) {
                JOptionPane.showMessageDialog(fixedGrowthPage,
                        "Interest Rate must be a postive number (numbers greater than 0)", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }

            if (localRate > 1000) {
                JOptionPane.showMessageDialog(fixedGrowthPage, "Rate is too large. Please use 1000 or less.", "Warning",
            ...
            if (localYears <= 0) {
                JOptionPane.showMessageDialog(fixedGrowthPage,
                        "Years must be a postive number (numbers greater than 0)", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }

            if (localYears > 1000) { 
                JOptionPane.showMessageDialog(fixedGrowthPage, "Target Years is too large. Please use 1000 or less.",
                        "Warning", JOptionPane.ERROR_MESSAGE);
                return;
            }

            // FIX #2: Standardized rate logic. Always divide by 100.
            // This assumes user always enters 5 for 5%.
            localRate = localRate / 100.0;

            localEndBalance = fixedInvestor(localPrincipal, localRate, localYears);

            endBalValue.setText("$" + String.format("%.2f", localEndBalance));

            // Builds and create chart upon successful submission
            JFreeChart chart = buildFixedInvestmentChart(localPrincipal, localRate, localYears);
        ...
        clearfBtn.addActionListener(e -> {
            ...
            JLabel msg = new JLabel("No chart to display. Enter values and click Calculate.", SwingConstants.CENTER);
        ...
        calcuBtn.addActionListener(e -> {
            ...
            double localPrincipal;
            try {
                localPrincipal = Double.parseDouble(principalField.getText().trim());
                if (localPrincipal < 0)
                    throw new NumberFormatException("Principal must be non-negative.");
            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(principalField, "Please enter a valid, non-negative Initial Investment.",
            ...
            if (rows == 0) {
                JOptionPane.showMessageDialog(rateTable, "No rates found. Click 'Generate Rate Table' and enter rates.",
            ...
            for (int r = 0; r < rows; r++) {
                ...
                double rateVal;
                try {
                    rateVal = (rateObj instanceof Number) ? ((Number) rateObj).doubleValue()
                ...
                //Standardized rate logic. Always divide by 100 for variable rates too.
            ...
            if (!badRows.isEmpty()) {
                ...
                JOptionPane.showMessageDialog(rateTable, errors.toString(), "Validation Errors",
                        JOptionPane.ERROR_MESSAGE);
                return;
            } else {
            ...
            double finalBalance = variableInvestor(localPrincipal, rateList);
            endBalValueV.setText(String.format("$%,.2f", finalBalance));

            ChartPanel chartPanel = buildVariableXYChartFromRates(localPrincipal, rateList);
        ...
        generateBtn.addActionListener(e -> {
            ...
            if (yearsText.isEmpty()) {
                JOptionPane.showMessageDialog(leftVar, "Please enter the number of years.", "Input Required",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            int localYears;
            try {
                localYears = Integer.parseInt(yearsText);
                if (localYears <= 0) {
                ...
                JOptionPane.showMessageDialog(leftVar, "Please enter a valid positive integer for years.",
                        "Invalid Input", JOptionPane.ERROR_MESSAGE);
                return;
            }
        ...
        // Uses addToGridBag method to add components to left side of Retirement Depletion
        // Page
        ...
        calculate.addActionListener(e -> {
            ...
            // Local variables
            double localPrincipal = 0;
            double localExpense = 0;
            double localRate = 0;

            // Validation Check to see if one or more fields are empty.
            if (balanceField.getText().compareTo("") == 0 || expenseField.getText().compareTo("") == 0
                    || rateField.getText().compareTo("") == 0) {
                JOptionPane.showMessageDialog(retireExpenPage, "One or more fields are empty", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            // Validation Check to see if the values in the fields are in number format
            try {
                localPrincipal = Double.parseDouble(balanceField.getText());
                localExpense = Double.parseDouble(expenseField.getText());
                localRate = Double.parseDouble(rateField.getText());
            ...
            if (localExpense <= 0) {
                JOptionPane.showMessageDialog(fixedGrowthPage,
                        "Expense must be a postive number (numbers greater than 0)", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }

            if (localExpense > localPrincipal) {
                JOptionPane.showMessageDialog(fixedGrowthPage,
                        "Expense cannot be higher than balance\nPlease enter either a lower expense or higher balance",
                        "Warning", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            int years = finallyRetired(localPrincipal, localExpense, localRate, 120);
            ...
            List<Double> series = balanceSeries(localPrincipal, localExpense, localRate);
        ...
        // Uses addToGridBag method to add components to left side of Optimization
        // Withdrawal Page
        ...
        // Uses addToGridBag method to add components to right side of Optimization
        // Withdrawal Page
        ...
        calculateOp.addActionListener(e -> {
            ...
            if (initialBalField.getText().compareTo("") == 0 || returnRateField.getText().compareTo("") == 0
                    || targetYearField.getText().compareTo("") == 0) {
                JOptionPane.showMessageDialog(retireExpenPage, "One or more fields are empty", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            try {
                localBalance = Double.parseDouble(initialBalField.getText());
                localRate = Double.parseDouble(returnRateField.getText());
                localYears = Integer.parseInt(targetYearField.getText());

            } catch (NumberFormatException nf) {
                JOptionPane.showMessageDialog(optimizePage, "Invalid datatype. All fields must be in number format.",
                        "Warning", JOptionPane.WARNING_MESSAGE);
                return;
            }

            // Validation check for balance, rate, years to see if they are non-negative
            // numbers
            if (localBalance <= 0) {
                JOptionPane.showMessageDialog(optimizePage,
                        "Investment must be a postive number (numbers greater than 0)", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (localRate <= 0) {
                JOptionPane.showMessageDialog(optimizePage,
                        "Return Rate must be a postive number (numbers greater than 0)", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (localRate > 1000) {
                ...
                JOptionPane.showMessageDialog(optimizePage, "Rate is too large. Please use 1000 or less.", "Warning",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            if (localYears <= 0) {
                JOptionPane.showMessageDialog(optimizePage,
                        "Target Years must be a postive number (numbers greater than 0)", "Warning",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (localYears > 1000) { 
                ...
                JOptionPane.showMessageDialog(optimizePage, "Target Years is too large. Please use 1000 or less.",
                        "Warning", JOptionPane.ERROR_MESSAGE);
                return;

            }

            // Standardized rate logic. Always divide by 100.
            localRate = localRate / 100.0;
           

            double withdrawal = maximumExpensed(localBalance, localRate, localYears);

            annualWithdrawlValue.setText("$" + String.format("%.2f", (withdrawal)));

            // 1) Find optimal spending via binary search
            double optimalSpend = maximumExpensed(localBalance, localRate, localYears);

            // 2) Build the balance history for the optimal spending
            List<Double> history = retirementHistory(localBalance, optimalSpend, localRate, 120);
            
            //Added Innovation/Visual Polish in chart building
            ...
            // 4) Update ONLY the scroll pane’s viewport (do not remove right panel or the
            // finalYear label)
    ...
    public static JFreeChart buildFixedInvestmentChart(double principal, double rate, int years) {
        // Rate is already converted in the Listener, but as a utility we trust the input.
        ...
        JFreeChart chart = ChartFactory.createLineChart("Investment Growth Over Time", "Year", "Balance ($)", dataset);
    ...
    private ChartPanel buildVariableXYChartFromRates(double principal, double[] rateList) {
        ...
        JFreeChart chart = ChartFactory.createXYLineChart("Variable Rate Growth", "Years", "Account Balance ($)",
                dataset);
    ...
    public static JFreeChart buildOptimalWithdrawalChart(List<Double> history, double optimalSpend) {
        ...
        XYSeries balanceSeries = new XYSeries("Balance");
        //Added Innovation/Visual Polish - Cumulative Withdrawal Series
        XYSeries withdrawalSeries = new XYSeries("Cumulative Cash Withdrawn");

        double cumulativeCash = 0;
        
        for (int year = 0; year < history.size(); year++) {
            balanceSeries.add(year, history.get(year));
            
            // Only add withdrawal if year > 0 (no withdrawal at year 0 start)
            if (year > 0) {
                cumulativeCash += optimalSpend;
            }
            withdrawalSeries.add(year, cumulativeCash);
        }
        ...
        XYSeriesCollection dataset = new XYSeriesCollection();
        dataset.addSeries(balanceSeries);
        dataset.addSeries(withdrawalSeries); // Add the new series
        dataset.addSeries(zeroSeries);
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(title, "Years", "Value ($)", dataset);
        ...
        // Renderer: series 1 (cumulative withdrawal) as BLUE line
        renderer.setSeriesPaint(1, new Color(0, 0, 139)); // dark blue
        renderer.setSeriesStroke(1, new java.awt.BasicStroke(2.0f));
        renderer.setSeriesShape(1, new Ellipse2D.Double(-3, -3, 6, 6)); // smaller markers

        // Renderer: series 2 (zero baseline) as thin red line, no markers
        renderer.setSeriesPaint(2, new Color(198, 40, 40)); // red
        renderer.setSeriesStroke(2, new java.awt.BasicStroke(1.5f));
    ...
    public void addToGridBag(JPanel panel, Component component, GridBagConstraints gbc, int column, int row,
